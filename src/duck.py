...
def start_duck(window):
    pygame.init()
    fps = 60
    timer = pygame.time.Clock()
    global font
    font = pygame.font.Font('res/font/myFont.ttf', 32)
    global big_font
    big_font = pygame.font.Font('res/font/myFont.ttf', 60)
    global WIDTH
    WIDTH = 1280
    global HEIGHT
    HEIGHT = 720
    ...
    global write_values
    write_values = False
    global new_coords , one_coords, three_coords
    new_coords = True
    one_coords = [[], [], []]
    two_coords = [[], [], []]
    three_coords = [[], [], [], []]
    global menu_img
    menu_img = pygame.image.load(f'res/menus/mainMenu.png')
    global game_over_img
    game_over_img = pygame.image.load(f'res/menus/gameOver.png')
    global pause_img
    pause_img = pygame.image.load(f'res/menus/pause.png')
    for i in range(1, 4):
        bgs.append(pygame.image.load(f'res/bgs/{i}.png'))
        banners.append(pygame.image.load(f'res/banners/{i}.png'))
        guns.append(pygame.transform.scale(pygame.image.load(f'res/guns/{i}.png'), (100, 100)))
        if i < 3:
            for j in range(1, 4):
                target_images[i - 1].append(pygame.transform.scale(
                    pygame.image.load(f'res/targets/{i}/{j}.png'), (120 - (j * 18), 80 - (j * 12))))
        else:
            for j in range(1, 5):
                target_images[i - 1].append(pygame.transform.scale(
                    pygame.image.load(f'res/targets/{i}/{j}.png'), (120 - (j * 18), 80 - (j * 12))))
    file = open('duck_high_scores.txt', 'r')
    ...
    pygame.mixer.init()
    pygame.mixer.music.load('res/sounds/bg_music.mp3')
    global plate_sound
    plate_sound = pygame.mixer.Sound('res/sounds/Broken plates.wav')
    plate_sound.set_volume(.2)
    global bird_sound
    bird_sound = pygame.mixer.Sound('res/sounds/Drill Gear.mp3')
    bird_sound.set_volume(.2)
    global laser_sound
    laser_sound = pygame.mixer.Sound('res/sounds/Laser Gun.wav')
    ...
    while run:
        ...
        if level > 0:
            ...
            if (level == 3 and target_boxes == [[], [], [], []]) or (mode == 1 and ammo == 0) or (
                ...
                game_over = True
        if write_values:
            file = open('duck_high_scores.txt', 'w')
